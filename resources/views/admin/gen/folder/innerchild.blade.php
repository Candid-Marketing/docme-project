    ...
    @endif
        <ul class="list-group folder-list">
            ...
            <li class="list-group-item d-flex justify-content-between align-items-center" data-folder-id="{{ $folder->id }}">
                    <a href="#" class="folder-link" onclick="submitFolder('{{$main_id}}', '{{$sub_id}}','{{$inner_id}}','{{$id_main}}','{{ $folder->id }}', '{{ $folder->innerchild_folder_name }}')">
        ...
        <form id="folderForm" action="{{ route('admin.lastfolders.show') }}" method="POST" style="display: none;">
            @csrf
