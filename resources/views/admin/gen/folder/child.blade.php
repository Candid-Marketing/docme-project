    ...
    @endif
        ...
        <ul class="list-group folder-list">
            ...
            <li class="list-group-item d-flex justify-content-between align-items-center" data-folder-id="{{ $folder->id }}">
                    <a href="#" class="folder-link" onclick="submitFolder('{{$main_id}}', '{{$sub_id}}','{{$id_main}}','{{ $folder->id }}', '{{ $folder->child_folder_name }}')">
        ...
        <form id="folderForm" action="{{ route('admin.innerchildfolders.show') }}" method="POST" style="display: none;">
            @csrf
