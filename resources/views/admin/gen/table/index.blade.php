        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="fileUploadForm" enctype="multipart/form-data">
                    @csrf
                    <!-- Hidden Inputs -->
                    <input type="hidden" name="main_id" id="mainId" value="{{ $main_id }}">
                    <input type="hidden" name="sub_id" id="subId" value="{{ $sub_id }}">
                    <input type="hidden" name="inner_id" id="innerId" value="{{ $inner_id }}">
                    <input type="hidden" name="folder_id" id="folderId" value="{{ $id_main }}">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="fileEditForm" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        <label for="editFileName" class="form-label">Replace File Name</label>
...
<div class="modal fade" id="inviteModal" tabindex="-1" aria-labelledby="inviteModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="inviteModalLabel">Invite to View File</h5>
            ...
            <div class="modal-body">
                ...
                <p><strong>Folder Name:</strong> <span id="folderName"></span></p>
...
<script>
    ...
    function uploadFile() {
        ...
        formData.append("main_id", document.getElementById("mainId").value);
        formData.append("sub_id", document.getElementById("subId").value);
        formData.append("inner_id", document.getElementById("innerId").value);
        formData.append("folder_id", document.getElementById("folderId").value);
        ...
        axios.post("{{ route('admin.files.upload') }}", formData, {
            headers: { "Content-Type": "multipart/form-data" }
        })
        .then(function (response) {
            // Use SweetAlert to show a message and refresh the page when the user clicks "OK"
            Swal.fire({
                title: 'Success!',
                ...
                if (result.isConfirmed) {
                    // Reload the page (or refresh the table data if needed)
    ...
    function updateFile() {
        ...
        axios.post(`{{ route('admin.files.update') }}`, formData, {
            headers: { "Content-Type": "multipart/form-data" }
        })
        .then(response => {
            // Hide spinner
    ...
    document.getElementById("searchInput").addEventListener("keyup", function () {
        ...
        const mainId = @json($main_id);
        const subId = @json($sub_id);
        const innerId = @json($inner_id);
        const childId = @json($id_main);

        if (query.length > 0) {
            axios.get("{{ route('admin.files.search') }}", {
                params: {
                    query: query,
                    main_id: mainId,
                    sub_id: subId,
        ...
        button.addEventListener('click', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.post(`{{ route('admin.files.delete') }}`, {
                        id: fileId,
                        _token: '{{ csrf_token() }}'
                    })
                    .then(response => {
