...
<div class="main">
    ...
    @endif
        ...
        <div class="table-container">
            <table>
                ...
                <tbody>
                    @forelse($folders as $folder)
                        <tr>
                            <td>
                                <a href="#" class="folder-name" onclick="submitFolder('{{$main_id}}','{{$id_main}}','{{ $folder->id }}', '{{ $folder->inner_folder_name }}')">
                                    <span class="folder-icon">📁</span> {{ $folder->inner_folder_name }}
                                </a>
                            </td>
                            <td>
                                <button class="btn btn-sm" style="background-color: #683695; color: white;"
                                    onclick="editSubFolder('{{$main_id}}','{{$id_main}}','{{ $folder->id }}', '{{ $folder->inner_folder_name }}', '{{ $folder->inner_folder_description }}')">
                                    ✏️ Edit
                                    </button>

                                    <!-- Delete Button (Pink) -->
                                    <button class="btn btn-sm" style="background-color: #ed1d7e; color: white;"
                                    onclick="deleteSubFolder('{{$main_id}}','{{$id_main}}','{{ $folder->id }}')">
                                    🗑 Delete
                                    </button>
                            </td>
                        </tr>
            ...
            <form id="paginationForm" action="{{ route('superadmin.innerfolders.show') }}" method="POST">
                @csrf
        ...
        <form id="folderForm" action="{{ route('superadmin.childfolders.show') }}" method="POST" style="display: none;">
            @csrf
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addFolderModalLabel">➕ Add Inner Folder</h5>
            ...
            <div class="modal-body">
                <form id="addSubFolderForm">
                    @csrf
                    <div class="mb-3">
                        <label for="folderName" class="form-label">Inner Folder Name</label>
                        <input type="hidden" name="sub_id" id="idMain" value="{{ $id_main }}">
                        <input type="hidden" name="main_id" id="mainIdValue" value="{{ $main_id }}">
                        <input type="text" class="form-control" name="inner_folder_name" id="innerfolderName" placeholder="Enter innerfolder name" required>
                    </div>
                    <div class="mb-3">
                        <label for="description" class="form-label">Inner Folder Description</label>
                        <input type="text" class="form-control" name="inner_folder_desc" id="description" placeholder="Enter innerfolder description" required>
                    </div>
...
<script>
    ...
    function submitFolder(mainId,subId,folderId, folderName) {
    ...
    function editSubFolder(mainFolder,subfolderID, folderId, folderName, folderDesc) {
    $("#editmainID").val(mainFolder);
    $("#editsubfolderID").val(subfolderID);
    $("#editInnerFolderId").val(folderId);
    ...
    $("#editInnerFolderForm").submit(function (event) {
        ...
        let formData = {
            ...
            folder_id: $("#editInnerFolderId").val(),
            sub_folder_name: $("#editInnerFolderName").val(),
            sub_folder_desc: $("#editInnerDescription").val()
        };

        $.post("{{ route('superadmin.update.innerfolders') }}", formData, function (response) {
    ...
    function deleteSubFolder(mainId, subId, folderId) {
        Swal.fire({
            title: "Are you sure?",
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('superadmin.delete.innerfolders') }}",
                    type: "POST",
                    data: {
