...
<div class="main">
    ...
    @endif
        ...
        <div class="table-container">
            <table>
                ...
                <tbody>
                    @forelse($folders as $folder)
                        <tr>
                            <td>
                                <a href="#" class="folder-name" onclick="submitFolder('{{$main_id}}', '{{$sub_id}}','{{$id_main}}','{{ $folder->id }}', '{{ $folder->child_folder_name }}')">
                                    <span class="folder-icon">📁</span> {{ $folder->child_folder_name }}
                                </a>
                            </td>
                            <td>
                                <button class="btn btn-sm" style="background-color: #683695; color: white;"
                                onclick="editSubFolder('{{$main_id}}', '{{$sub_id}}','{{$id_main}}','{{ $folder->id }}', '{{ $folder->child_folder_name }}', '{{ $folder->child_folder_description }}')">
                                ✏️ Edit
                                </button>

                                <!-- Delete Button (Pink) -->
                                <button class="btn btn-sm" style="background-color: #ed1d7e; color: white;"
                                onclick="deleteSubFolder('{{$main_id}}', '{{$sub_id}}','{{$id_main}}','{{ $folder->id }}')">
                                🗑 Delete
                                </button>
                            </td>
                        </tr>
            ...
            <form id="paginationForm" action="{{ route('superadmin.childfolders.show') }}" method="POST">
                @csrf
                <input type="hidden" name="main_id" value="{{ $main_id }}">
        ...
        <form id="folderForm" action="{{ route('superadmin.innerchildfolders.show') }}" method="POST" style="display: none;">
            @csrf
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addFolderModalLabel">➕ Add Child Folder</h5>
            ...
            <div class="modal-body">
                <form id="addSubFolderForm">
                    @csrf
                    <div class="mb-3">
                        <label for="folderName" class="form-label">Child Folder Name</label>
                        <input type="hidden" name="sub_id" id="idMain" value="{{ $sub_id }}">
                        <input type="hidden" name="main_id" id="mainIdValue" value="{{ $main_id }}">
                        <input type="hidden" name="child_id" id="childId" value="{{ $id_main }}">
                        <input type="text" class="form-control" name="inner_folder_name" id="innerfolderName" placeholder="Enter childfolder name" required>
                    </div>
                    <div class="mb-3">
                        <label for="description" class="form-label">Child Folder Description</label>
                        <input type="text" class="form-control" name="inner_folder_desc" id="description" placeholder="Enter childfolder description" required>
                    </div>
...
<script>
    ...
    function submitFolder(mainId,subId,innerId,folderId, folderName) {
    ...
    $(document).ready(function () {
        $("#addSubFolderForm").submit(function (event) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('superadmin.add.childfolders') }}", // Ensure this route matches your controller method
                data: formData,
    ...
    function editSubFolder(mainFolder,subfolderID,innerId, folderId, folderName, folderDesc) {
    $("#editmainID").val(mainFolder);
    $("#editsubfolderID").val(subfolderID);
    $("#editInnerFolderId").val(innerId);
    $("#editchildfolderID").val(folderId);
    ...
    $("#editInnerFolderForm").submit(function (event) {
        ...
        $.post("{{ route('superadmin.update.childfolders') }}", formData, function (response) {
    ...
    function deleteSubFolder(mainId, subId, innerId, folderId) {
        Swal.fire({
            title: "Are you sure?",
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('superadmin.delete.childfolders') }}",
                    type: "POST",
                    data: {
