...
<div class="main">
    ...
    @endif
        ...
        <ul class="list-group folder-list">
            ...
            <li class="list-group-item d-flex justify-content-between align-items-center" data-folder-id="{{ $folder->file->inner->id }}">
                    <a href="#" class="folder-link" onclick="submitFolder('{{ $folder->file->inner->id }}', '{{ $folder->file->inner->inner_folder_name }}')">
                        <span class="folder-icon">📁</span> {{ $folder->file->inner->inner_folder_name }}
                    </a>
                </li>
            @empty
        ...
        <form id="folderForm" action="{{ route('user.childfolders.show') }}" method="POST" style="display: none;">
            @csrf
