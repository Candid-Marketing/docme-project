...
<div class="main">
    ...
    @endif
        ...
        <ul class="list-group folder-list">
            ...
            <li class="list-group-item d-flex justify-content-between align-items-center" data-folder-id="{{ $folder->file->innerChild->id }}">
                    <a href="#" class="folder-link" onclick="submitFolder('{{ $folder->file->innerChild->id }}', '{{ $folder->file->innerChild->innerchild_folder_name }}')">
                        <span class="folder-icon">📁</span> {{ $folder->file->innerChild->innerchild_folder_name }}
                    </a>
                </li>
            @empty
        ...
        <form id="folderForm" action="{{ route('user.lastfolders.show') }}" method="POST" style="display: none;">
            @csrf
